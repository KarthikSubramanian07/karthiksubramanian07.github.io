(function(){
  ...
  var IC={
    ...
    discord:'<path fill="currentColor" fill-rule="evenodd" d="M20.317 4.492c-1.53-.69-3.17-1.2-4.885-1.49a.075.075 0 0 0-.079.036c-.21.369-.444.85-.608 1.23a18.566 18.566 0 0 0-5.487 0 12.36 12.36 0 0 0-.617-1.23A.077.077 0 0 0 8.562 3c-1.714.29-3.354.8-4.885 1.491a.07.07 0 0 0-.032.027C.533 9.093-.32 13.555.099 17.961a.08.08 0 0 0 .031.055 20.03 20.03 0 0 0 5.993 2.98.078.078 0 0 0 .084-.026 13.83 13.83 0 0 0 1.226-1.963.074.074 0 0 0-.041-.104 13.175 13.175 0 0 1-1.872-.878.075.075 0 0 1-.008-.125c.126-.093.252-.19.372-.287a.075.075 0 0 1 .078-.01c3.927 1.764 8.18 1.764 12.061 0a.075.075 0 0 1 .079.009c.12.098.245.195.372.288a.075.075 0 0 1-.006.125c-.598.344-1.22.635-1.873.877a.075.075 0 0 0-.041.105c.36.687.772 1.341 1.225 1.962a.077.077 0 0 0 .084.028 19.963 19.963 0 0 0 6.002-2.981.076.076 0 0 0 .032-.054c.5-5.094-.838-9.52-3.549-13.442a.06.06 0 0 0-.031-.028zM8.02 15.33c-1.183 0-2.157-1.085-2.157-2.419 0-1.333.956-2.419 2.157-2.419 1.21 0 2.176 1.096 2.157 2.42 0 1.333-.956 2.418-2.157 2.418zm7.975 0c-1.183 0-2.157-1.085-2.157-2.419 0-1.333.955-2.419 2.157-2.419 1.21 0 2.176 1.096 2.157 2.42 0 1.333-.946 2.418-2.157 2.418z"/>',
  ...
  var stack=[
    {cat:'Languages',ic:IC.code,items:['Rust','Python','TypeScript','C++','C','Swift','Kotlin','Java','JavaScript','Scheme']},
    {cat:'Web & Realtime',ic:IC.layers,items:['Next.js','React','React Native','Electron','WebRTC','LiveKit','Three.js','Deck.gl','Mapbox','WebSocket','React Flow','Svelte','Framer Motion','Node.js','Tailwind CSS','Zustand','Radix UI','Vite','Zod']},
    {cat:'Backends & Data',ic:IC.cpu,items:['PostgreSQL','pgvector','GraphQL','FastAPI','Redis','PySpark','Polars','Dask','MongoDB','FFmpeg','Supabase','tRPC','Prisma','Drizzle','Express','Flask','MySQL','SQLite','Auth0','World ID']},
    {cat:'Cloud & Infra',ic:IC.tools,items:['AWS','Google Cloud','Kubernetes','Docker','Dagster','BigQuery','Cloudflare','Vercel','OpenTelemetry','Arize Phoenix','Browserbase','Stagehand','Sentry','Firebase','PostHog','Playwright','Railway','Retool','Make','Turborepo','nginx']},
    {cat:'AI / ML & Agents',ic:IC.ai,full:true,items:['PyTorch','CUDA','TensorFlow','XGBoost','YOLO','SAM','Depth Anything v2','OpenCV','Whisper','Claude','GPT','Gemini','Gemma','DeepSeek','Mistral','Llama','Qwen','Hugging Face','Sentence Transformers','fastembed','scikit-learn','LightGBM','ONNX','SHAP','pytesseract','Pydantic AI','Letta','Agent S','Fetch.ai uAgents','OpenRouter','AWS Bedrock','Groq','Ollama','Deepgram','ElevenLabs','spaCy']},
  ...
  var t0=performance.now(),cachedCol='',cachedBase=0,cachedCoralStr='',cachedDark=null;
  function refreshCache(){var dark=isDark();if(dark===cachedDark&&cachedCol)return;cachedDark=dark;
    cachedCol=cssv('--star')||'#fff';cachedBase=parseFloat(cssv('--starA'))||.85;cachedCoralStr=cssv('--coral')||'#E08A6B';}
  function frame(now){
    ...
    if(reduce){requestAnimationFrame(frame);return;}
    ...
    for(var i=0;i<stars.length;i++){var st=stars[i],tw=.35+.65*(.5+.5*Math.sin(st.p+(now-t0)*st.s)),px=st.x+tx*st.z*26,py=st.y+ty*st.z*26;
      ctx.beginPath();ctx.arc(px,py,st.r,0,6.283);ctx.fillStyle=hexA(cachedCol,cachedBase*tw*st.z);ctx.fill();
    ...
    if(near.length){ctx.lineWidth=.7;
      for(var a=0;a<near.length;a++){var d2=(near[a][0]-mx)*(near[a][0]-mx)+(near[a][1]-my)*(near[a][1]-my);
        ctx.strokeStyle=hexA(cachedCol,Math.max(0,(1-d2/20000)*(dark?.5:.4)));ctx.beginPath();ctx.moveTo(mx,my);ctx.lineTo(near[a][0],near[a][1]);ctx.stroke();
        for(var b=a+1;b<near.length;b++){var ddx=near[a][0]-near[b][0],ddy=near[a][1]-near[b][1],dd=ddx*ddx+ddy*ddy;
          if(dd<7200){ctx.strokeStyle=hexA(cachedCol,Math.max(0,(1-dd/7200)*(dark?.32:.26)));ctx.beginPath();ctx.moveTo(near[a][0],near[a][1]);ctx.lineTo(near[b][0],near[b][1]);ctx.stroke();}}}
      ctx.fillStyle=hexA(cachedCoralStr,dark?.9:.7);ctx.beginPath();ctx.arc(mx,my,2,0,6.283);ctx.fill();}
  ...
  }else{refreshCache();for(var i=0;i<stars.length;i++){var st=stars[i];ctx.beginPath();ctx.arc(st.x,st.y,st.r,0,6.283);ctx.fillStyle=hexA(cachedCol,.45);ctx.fill();}}
  requestAnimationFrame(frame);
})();
